...
target.addEventListener("click", (e) => {
  ...
  if (e.detail>= 3) e.stopPropagation(); // explicit stop propagation on triple click
}, false);
...
captureworkarea.addEventListener(
  "click",
  (e) => {
     printOutEvent("CAPTURE Workarea click ("+e.detail+")", "workarea");
  },
  true //so this one gets events on way down, so workarea will be BEFORE target
);
capturetarget.addEventListener("click", () => printOutEvent("capture rect - click "), false);
